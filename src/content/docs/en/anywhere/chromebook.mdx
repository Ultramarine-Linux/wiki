...
This guide assumes you are on x86 and have [Developer Mode](https://chromium.googlesource.com/chromiumos/docs/+/main/developer_mode.md#dev-mode) and [USB Booting Enabled](https://chromium.googlesource.com/chromiumos/docs/+/main/developer_guide.md#boot-from-your-usb-disk). We also assume you are using gparted for the partitioning tasks, but other tools will work too. There are alterations to these instructions you could make and still get the same end result, this is just what we find to be the easiest way.
...
2. Create a 16 MB ext4 partition at the END of the disk (for proper resizing after installation). In gparted, accomplish this by making a new partition in the unallocated space, then drag it all the way to the right. 
3. Flash the [submarine-x86.kpart](https://nightly.link/FyraLabs/submarine/workflows/build/main/submarine-x86_64.zip) file to the partition.
4. Set the partition flags with cgpt (may have to install `vboot-utils`).

```
cgpt add -i <Submarine partition number> -t kernel -P 15 -T 1 -S 1 /dev/sdX
```

5. Boot the image on the Chromebook.

You're now inside the Ultramarine install on your USB/SD card. Running slowly is standard behavior and is not reflective of the preformance once running off internal storage.

6. `dd` the USB/SD card to the internal disk of the Chromebook. (Note, you do NOT need to resize the root filesystem, it should do this automatically upon first boot).
...
7. Once the `dd` is done, verify the Submarine partition shows 'ChromeOS Kernel' as a flag on the internal disk. We recommend using GParted for this (dnf install gparted if needed).
