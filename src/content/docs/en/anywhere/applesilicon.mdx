...
We are steadlily working on porting the Asahi Linux installer to work with Ultramarine. Images that can boot from Asahi UEFI (using `kernel-16k` to align with Mac page size allocation requirements) are activly being worked on as well.
