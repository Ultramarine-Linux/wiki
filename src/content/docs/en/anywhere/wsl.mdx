...
<Alert type="warning">
    Ultramarine's WSL application is still is currently in review by the Microsoft Store team. You will not be able to install Ultramarine WSL with the below instructions until the application is aproved for the store. When Ultramarine WSL is live, this message will be removed.
</Alert>
...
- Restart your device, then enter your bios/setup key when the system comes back on. A list of these keys can be found [here](/en/setup/bioskeys).
...
- Save and exit. There will typically be a deticated key to do this, or a select option on the main or exit page.

Hardware virtualization should now be enabled, and you can proceed to installation.

## Installation

There are a few different ways to install a WSL distro. Below are what we recommended.
...
Then, open the Microsoft Store, and search 'Ultramarine Linux' in the Microsoft Store app.

Open the Microsoft Store, and search 'Ultramarine Linux' in the Microsoft Store app.
...
When you boot into Ultramarine WSL (or any WSL for that matter), you will be prompted to enter a username and password. Follow the prompts to create a user, then sign into your new user, and then you're all good to go!
...
Then, replace `Ultramarine` in the setdefault command to the listed name.
