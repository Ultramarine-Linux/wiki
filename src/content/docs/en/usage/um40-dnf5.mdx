...
advised to report this bug to upstreams (i.e. the apps in conern), not `dnf5` or Ultramarine.
...
- We hope this change will push apps in the Fedora neighbourhood to switch to `dnf5` and at the same
  time discover apps that are poorly maintained/written and relies on `dnf`/`dnf-3`.
...
- If changing only 2 symlinks actually breaks your workflows, either your workflows are poorly made,
  ...
  actual `dnf-3` binary; or in the latter case, report the issue to the upstream apps in conern.
