...
By default, Ultramarine uses the [Btrfs](https://en.wikipedia.org/wiki/Btrfs) filesystem (except on Chromebook Edition), which supports allocating the drive into multiple subvolumes. This is the recommended option for most users as this will allow you to back up your system as atomic snapshots, which take less space than a full data clone.
