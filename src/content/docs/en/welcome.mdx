...
While Fedora ships a primarily free system by default, Ultramarine works off the model of least resistance. We try to make it easy for the user, even if we have to ship some proprietary drivers, firmware, and codecs that Fedora wouldn’t be comfy with out-of-box.

### Ultramarine is Innovative.

We believe that recent innovations in the Linux space serve to greatly improve the user and developer experience. Technologies like Flatpak and xdg-portals allow developers to provide great experiences to our users more consistently. Work done upstream improves the experience for our users, and work done in the Ultramarine ecosystem; like accent colours and Chromebook support improves the larger Linux ecosystem.
...
Ultramarine Linux was created out of frustration with the legal challenges of Fedora. As Fedora's parent company Red Hat is an American entity, there are legal restrictions on what software can be
packaged in the distribution due to the US patent system.

Ultramarine, on the other hand, is based in Thailand and is not subject to any of the copyright restrictions on what software can be packaged, preferring to give users the choice of what software to use based on the laws of their jurisdiction.
...
### If some packages are updated mid-release, doesn't that mean Ultramarine is unstable/insecure compared to, e.g. Debian Stable?

**No**. That's an incorrect approach to view if a distribution is secure/stable or not.

- Packages in Fedora/Ultramarine are updated mid-release, but those updates **only include non-breaking changes**, mirroring the approach of releases such as Debian Stable by retaining ABI and API compatibility throughout the stable release cycle. Your computer has virtually zero chance of breaking mid-release due to an update.
- Undeniably, Ultramarine and Fedora has much more frequent updates compared to other stable releases, and to some extent, they might be more unstable. However, Fedora/Ultramarine's release cycle makes it so that the operating system is still stable enough for normal use. Millions of Fedora devices across the world run fine without breaking, which demonstrate Fedora's high stability. Debian Stable's release cycle is also arguably too slow for desktop usage and can cause more hassles than necessary.
- Some distros, primarily enterprise, choose to take stable release cycles to an extreme. These distros are designed for servers that will stay up for years by delaying updates for months or even a few years. Desktop users might miss out on important features and bug fixes with this release model. Without the proper knowledge and management, these systems could be even more insecure than Fedora/Ultramarine.

### Why Ultramarine (instead of other distros)

- Performance
  - Fedora, the base of Ultramarine, has some of the best I/O performance over other distros.
  - BTRFS is the default file system. It is a relatively performant and feature-complete file system with features like snapshots and improved performance on solid-state media. You can backup your files in less than a second (unlike other file systems like ext4).
  - We include lightweight desktop environments such as Budgie (Flagship Edition) and XFCE. They look great and offer solid performance and usability.
- Privacy
  - We don't track/collect your private/personal data.
  - We don't even know how many Ultramarine devices there are, in contrast to other distros with primarily corporate backing.
- Security
  - Security updates are provided quickly while allowing new features to be added mid-release with little to no breakage.
...
- Usability
  ...
  - Ultramarine has sane defaults.
    - Popular packages that the average user might need are installed by default, including proprietary codecs, drivers, firmware, etc.
...
- Performance
  - Windows ships many different potentially unwanted programs that slow down your computer. Ultramarine does not.
  - Ultramarine provides a few lightweight desktop environments (namely Budgie/Flagship and XFCE) that offer more control to the average user without harming performance and usability.
  - Ultramarine uses newer technologies that are not available on Windows because of backwards compatibility. These technologies bring better performance.
...
- Privacy
  - We do not collect/track personally identifiable information. We have no use for it.
  - We have no way of tracking how many people use Ultramarine.
  - When you setup Windows, you need to accept a EULA (end-user license agreement) that is intentionally difficult to follow. Ultramarine uses free software licenses while providing the user with the option to use proprietary software for the sake of usability.
  - Ultramarine is open source. Anyone can check if we track/collect your data by reviewing the [source code](https://github.com/Ultramarine-Linux).
- Security
  - Security updates are more frequent and stable than Windows.
    - Most userland updates don't require reboots.
  - It's much more unlikely for malware, viruses, etc. to target Linux due to its current market share.
  - Our open source model makes it very easy for anyone to spot and fix bugs/vulnerabilities
- Stability
...
- Customizations and Usability
  ...
  - We support reasonably old and new 64-bit hardware, and we provide many tools for you to setup a performant, personalized system on your computer.
