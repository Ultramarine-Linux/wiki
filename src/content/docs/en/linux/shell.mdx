...
The directory path is an optional agrument. Running `ls` on its will print the contents of the current working directory (the one you  `cd`ed into).
...
| `-d` | List specified directory without listing it's contents, (try `ls -d /`).        |
...
The output of `ls -l` includes information on the file type, target, read-only (boolean), permissions, number of symlinks, number of inodes, and the owner of the file, in that order.
```
total 4
...
Symlinks are, as the name would imply, links between files. Where a symlink occurs, changes are synced between files. To create a symlink bewtween two files, type `ln`, followed by the `-s` flag, and two arguments: one being the first file, and the other being the new linked file.
