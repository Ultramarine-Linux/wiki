---
title: Software and Package Mnagement
...
Unlike Windows, most software for Ultramarine Linux is stored in online collections of software called repositories. You can browse and install software from these repositories using something called a package manager.

### Flatpaks

Flatpak is the primary package manager for regular apps. It installs applications in a sandbox, keeping them seperate from anything they don't need to access.

Flatpaks can be installed from **Software** on Budgie and GNOME, **AppCenter** on Pantheon, and **Discover** on KDE.

You can browse the available software in one of these app stores, or on the [Flathub website](flathub.org).

Despite most applications being installed from Flatpak, some apps may not work properly, or may not be availible at all. The following sections will help with software in other formats.
...
#### AppImages
1. Install AppImage Launcher from **Software** or **Discover**. If using Pantheon Edition, run 
```
...
Snap packages load slow and may not work at all on editions other than GNOME. We recommend avoiding Snaps.
...
#### [← Back To: Permissions](permissions)

#### [Next Up: The Shell →](shell)
