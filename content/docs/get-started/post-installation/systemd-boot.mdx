---
title: Use systemd-boot as the Bootloader (Advanced)
description: Try a modern bootloader
---
...
<Callout type="warning">
  ...
  Make sure to back up your data and have a live USB ready in case
...
1. Make sure you turn off Secure Boot in your UEFI settings for now, You can re-enable it later after [Setting up Secure Boot](#using-secure-boot-with-systemd-boot)
...
4. Install `systemd-boot-unsigned`<sup>dnf</sup> and `sdubby`<sup>dnf</sup>:
...
7. (Optional) Proceed to [Set up Secure Boot](#using-secure-boot-with-systemd-boot) if you want to re-enable Secure Boot.

## Using Secure Boot with systemd-boot
...
Fortunately, there's a tool to automate this called [sbctl]<sup>dnf</sup>. You can install it from the Terra repository:
