...
You may be familiar with bash from other Linux distributions. Ultramarine uses the [zsh](https://en.wikipedia.org/wiki/Z_shell) instead of bash. We also use the [Starship Prompt](https://starship.rs/)<sup>dnf</sup> to provide additional convenience features like displaying the current git branch.
