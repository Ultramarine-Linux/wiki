...
- Ultramarine Hop (our tool for adding/removing/switching editions and desktop environments) may not work. The new version is blocked by [an upstream bug from dnf5](https://github.com/rpm-software-management/dnf5/issues/2510).
- Anaconda Web Installer shows "Fedora" on the first page. This is fixed [in upstream](https://github.com/rhinstaller/anaconda-webui/pull/1082).
...
- Nvidia and Surface users may experience a kernel panic after upgrade.
  ...
  This is caused by the boot partition running out of space, you can boot into the previous kernel entry and either install `dracut-strip-trigger` (which will break moving drives to another device) or [tell DNF to keep less kernels](https://discussion.fedoraproject.org/t/how-can-i-change-the-number-of-kernels-retained-when-updating-in-fedora/73863/2).

  Since Ultramarine 42, only 2 kernel versions are kept, and `dracut-strip-trigger`<sup>dnf</sup> should be installed by default.

- Taidan language switching does not work
...
- Nvidia and Surface users may experience a kernel panic after upgrade.
  ...
  This is caused by the boot partition running out of space, you can boot into the previous kernel entry and either install `dracut-strip-trigger`<sup>dnf</sup> (which will break moving drives to another device) or [tell DNF to keep less kernels](https://discussion.fedoraproject.org/t/how-can-i-change-the-number-of-kernels-retained-when-updating-in-fedora/73863/2).


## Ultramarine Linux 40 (EOL)
...
- Flathub not included in new installs
  ...
  Our builder had dnf weakdeps disabled, causing `ultramarine-repos-extras`<sup>dnf</sup> to not be installed. This was fixed in UM40.1.
...
- Fedora bookmarks still included
  ...
  We removed the Fedora bookmarks and replaced them with `ultramarine-phony-bookmarks`<sup>dnf</sup>. As the name implies, there are no bookmarks.
...
The Pop!\_Shell launcher is not included. This can be fixed by installing the `pop-launcher`<sup>dnf</sup> package.
